...
// Validate the email address using DataAnnotations like the UserValidator does when RequireUniqueEmail = true.
        ...
        where TUser : User, new()
    {
            ...
            ([FromBody] LoginRequest login,  [FromServices] IServiceProvider sp) =>
        {
            ...
            if(user !=null && await signInManager.UserManager.CheckPasswordAsync(user, login.Password)){
                var token = GenerateJwtToken(user, configs);
            ...
            (ClaimsPrincipal claimsPrincipal, [FromServices] IServiceProvider sp) =>
        {
            ...
            //     var user = await userManager.FindByEmailAsync(claimsPrincipal.Claims[])
            // }
    ...
    private static string GenerateJwtToken(User user, IConfiguration configs)
    {
        var claims = new[]
        {
        ...
        var key = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(configs["JWT:SignInKey"]));
        var creds = new SigningCredentials(key, SecurityAlgorithms.HmacSha256);
        var token = new JwtSecurityToken(
            issuer: configs["JWT:Issuer"],
            audience: configs["JWT:Audience"],
            claims: claims,
            expires: DateTime.UtcNow.AddMinutes(configs.GetValue<int>("JWT:ExpirationInMinutes")),
